...
describe('REDUCER:', () => {
  ...
  describe('buttonBarsReducer.js', () => {
    ...
    it('should return the current state when action actionConstants is of unknown', () => {
      expect(reducer(initialState, {})).toEqual(fromJS({ buttons: List([]), bars: List([]) }));
    });

    describe('ACTIONS.BUTTON_BARS.FETCH', () => {
      it('should return the current state because this does nothing', () => {
        expect(reducer(initialState, ACTIONS.fetchData())).toEqual(fromJS({ buttons: List([]), bars: List([]) }));
      });
      it('should return the current state because this does nothing', () => {
        expect(reducer(initialState, ACTIONS.fetchData(fromJS({
          buttons: List([1]),
          bars: List([1])
        })))).toEqual(fromJS({ buttons: List([]), bars: List([]) }));
      });
    });
    ...
    describe('ACTIONS.BUTTON_BARS.SAVE', () => {
      it('should return the current state because this does nothing', () => {
        ...
        expect(x).toEqual(fromJS({ buttons: List([]), bars: List([]) }));
      });
      it('should return return the initial state', () => {
        expect(reducer(initialState, ACTIONS.saveData(initialState))).toEqual(fromJS({
          buttons: List([]),
          bars: List([])
        }));
      });
